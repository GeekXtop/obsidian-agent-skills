...
`$oblearn` 只负责从当前项目提取可复用知识；不确定归类的新知识可建议写入 `Agent/Knowledge/Inbox/`，并在关键词明确时更新 `_catalog.md`。`$obcurate` 负责低频整理公共知识库，例如清理 Inbox、维护 `_catalog.md`、合并或拆分主题、修正 aliases/tags/wikilink。
...
校验脚本会检查 skill frontmatter、OpenAI agent metadata、Claude slash command、模板语言回归规则，以及插件 manifest 版本一致性。
