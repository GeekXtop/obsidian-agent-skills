...
| [oblearn](skills/oblearn) | 从项目内经验提取跨项目可复用知识，并写入或建议写入 Obsidian 公共知识笔记。 |
...
`$oblearn` 只负责把当前项目经验提取为跨项目公共知识；项目内经验先由 `$obclose` 写入 `.agents/lessons.md`。不确定归类的新知识可建议写入 `Agent/Knowledge/Inbox/`，并在关键词明确时更新 `_catalog.md`。`$obcurate` 负责低频整理公共知识库，例如清理 Inbox、维护 `_catalog.md`、合并或拆分主题、修正 aliases/tags/wikilink。
