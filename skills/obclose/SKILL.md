...
新条目一律**追加到文件末尾**，文件按时间升序排列：最新条目在最下方。不要按日期把条目插入中间，也不要把新条目放到顶部——一次性写入多条积压记录时尤其要注意，倒序写入会让文件永久分成排序不同的区块。派生路径同理：先写文件末尾的条目，再由它派生 `active.md`，不要为了迁就 `active.md` 的叙述顺序而回填到文件中部。
...
- `progress.md` 过长时，保留最近阶段记录，把较旧阶段压缩为短摘要，或归档到 `.agents/archive/progress-YYYY.md`。归档时按连续区块整体移出，并让归档文件内部**按时间升序排列**——现行文件与归档文件都升序，新旧之分只体现在哪个文件里。
