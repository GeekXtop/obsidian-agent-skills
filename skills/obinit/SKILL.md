...
| 成熟项目接入模式 | fork、已有项目、已有非空 `AGENTS.md` / `CLAUDE.md` / README / docs | 使用 `templates/instructions-index.md` 创建索引型 `.agents/instructions.md`；保留已有指南，只追加中文入口提示 |
| 重复运行模式 | `.agents/instructions.md` 已存在，或入口提示已存在，或 `.agents/active.md` 已存在 | 幂等更新：只补缺失文件、缺失链接、缺失入口提示；不重写、不重复追加、不重置状态文件 |
