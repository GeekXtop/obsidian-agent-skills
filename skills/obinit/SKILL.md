...
任务开始或遇到相关问题时，不凭空假设哪些领域已有公共知识。只有用户明确要求、`Agent/Knowledge/_catalog.md` 的 `terms` / `aliases` 命中任务关键词，或风险较高且关键词明确时，才在 `Agent/Knowledge/` 做有限关键词定向搜索；只有命中相关笔记后才明确读取并使用。不要全量自动加载公共知识，不扫描整个 vault。
